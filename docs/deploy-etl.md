...
## Склейка тайлов: полнота и `<лист>_gaps.json` (раунды 12–14)

`segmentate.py` собирает канвас листа из тайлов в его папке. Раньше сетка
сборки (число строк/столбцов) выводилась подсчётом присутствующих файлов —
единичный пропавший тайл ронял склейку исключением, а пропажа целого
X/Y-столбца могла тихо сдвинуть результат без единой ошибки в логе (диагноз —
`docs/round12-saturation-verdict.md`, задача 4; воспроизведение и фикс —
`docs/round13-threshold-outcome.md` и `docs/round14-final-detector.md`).

Сейчас сетка строится по **истинной границе листа из геометрии `razgrafka`**
(`SegmentationTask.get_sheet_tile_range()`), а не по тому, что физически на
диске. Практические следствия:

- Недостающий тайл (в любом месте, включая целый неохваченный край листа)
  заполняется чёрным плейсхолдером на своей истинной позиции — остальные
  тайлы никогда не сдвигаются, и канвас всегда полного (истинного) размера
  листа, даже если бо́льшая часть тайлов отсутствует.
- Если пропуски есть — рядом с папкой листа (не внутри неё, по аналогии с
  `_missing.json`) пишется **`<лист>_gaps.json`**:
  ```json
  {
    "z": 17, "min_x": ..., "max_x": ..., "min_y": ..., "max_y": ...,
    "expected_tiles": 672, "missing_count": 398,
    "missing_fraction": 0.5923,
    "missing": [{"z": 17, "x": ..., "y": ..., "reason": "no_tile_file"}, ...]
  }
  ```
  `missing_fraction` — доля плейсхолдер-пикселей на листе (тот же лог уровня
  `WARNING` от `process_trapeze()` при каждом прогоне, пока лист не докачан
  полностью). Проверить, у каких листов есть пропуски:
  ```bash
  find outputs/tiles -maxdepth 1 -name '*_gaps.json'
  ```
- Если геометрия листа не нашлась в `razgrafka` — откат на старое поведение
  (диапазон выводится из присутствующих файлов, тот же риск тихого сдвига при
  пропаже целого края, что и до раунда 14, но пропуски внутри диапазона
  по-прежнему безопасны с раунда 13). В лог при этом пишется `ERROR`.

**Сверка полноты пакета**: `package.py` (`check_missing_sheets()`) при сборке
пакета сверяет TIF каждого года с полным списком листов проекта
(`trapeze_serv`, не `razgrafka` — последняя общенациональная и для этой сверки
не подходит) и пишет `ERROR` по отсутствующим — лист, не попавший в
`predictions/geoserver_public/{year}/`, больше не пропадает из поставки молча.
