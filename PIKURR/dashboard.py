...
with col_ctrl:
    ...
    # Шаг «9. Отправка на сервер» — отдельная кнопка «ОТПРАВИТЬ ПАКЕТ НА
    # СЕРВЕР» (push_btn/run_push), не часть полного цикла (раунд 16, задача
    # 4: убрана явная отправка пакета из «полного цикла», чтобы не дублировать
    # отдельную кнопку и не отправлять пакет неявно при обычном прогоне).
    # steps_ui[9] используется только в run_push(), своей плашки в гриде
    # полного цикла не заводит — отдельный st.empty() под неё создаётся там же.
    steps_ui[9] = st.empty()
...
def run_pipeline():
    ...
    try:
        ...
        with contextlib.redirect_stdout(StreamlitStdout(log_widget, log_buffer)):
            ...
            # Отправка на сервер («шаг 9») убрана из полного цикла (раунд 16,
            # задача 4) — дублировала отдельную кнопку «ОТПРАВИТЬ ПАКЕТ НА
            # СЕРВЕР» (push_btn/run_push) и отправляла пакет неявно при
            # обычном прогоне. Полный цикл теперь заканчивается на архивации.
