# Архивная копия src/services/inference.py ДО раунда 7
# (prompts/PROMPT_onnx_inference_round7.md) — реализация через ovmsclient/gRPC
# к TensorFlow Serving. Заменена на ONNX Runtime, потому что TensorFlow
# не поддерживает GPU архитектуру Blackwell (RTX 5070 Ti, sm_120) —
# см. project_pikurr_status.md и tensorflow/tensorflow#99592.
#
# Оставлена как путь отката: если понадобится вернуться на TF Serving,
# сервис tf-serving в docker-compose.yml не удалён (закомментирован),
# исходные SavedModel лежат рядом с .onnx в ${HOST_MODEL_DIR}.
# Этот файл не импортируется и не участвует в сборке — только референс.
